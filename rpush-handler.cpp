...
// rpush-handler.cpp
#include "../include/rpush-handler.h"
#include "../include/list-store.h"
...
std::string handle_rpush(std::vector<std::string> command_array) {
    ...
    {
        std::lock_guard<std::mutex> lock(list_store.mtx);
        auto &target_list = list_store.lists[key];   // creates an empty deque if key is new — stage 1!
    ...
    list_store.cv.notify_all(); // wake any BLPOP threads waiting on this key
